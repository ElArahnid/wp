...
if ( ! class_exists( 'Fitness_Exercise_Hub_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Fitness_Exercise_Hub_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.0.0
		 * @return array Returns an array of remote URLs and their local fitness_exercise_hub_counterparts.
		 */
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $fitness_exercise_hub_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $fitness_exercise_hub_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$fitness_exercise_hub_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$fitness_exercise_hub_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$fitness_exercise_hub_font_family = trim( str_replace( array( "'", ';' ), '', $fitness_exercise_hub_font_family ) );
					$fitness_exercise_hub_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $fitness_exercise_hub_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $fitness_exercise_hub_font_family ] ) ) {
					$result[ $fitness_exercise_hub_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$fitness_exercise_hub_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$fitness_exercise_hub_font_family_url = $this->get_absolute_path( $fitness_exercise_hub_font_family_url );

					$result[ $fitness_exercise_hub_font_family ][] = $fitness_exercise_hub_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $fitness_exercise_hub_font_family ] = array_flip( array_flip( $result[ $fitness_exercise_hub_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'fitness_exercise_hub_wptt_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function fitness_exercise_hub_wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Fitness_Exercise_Hub_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'fitness_exercise_hub_wptt_get_web_font_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function fitness_exercise_hub_wptt_get_web_font_url( $url, $format = 'woff2' ) {
		$font = new Fitness_Exercise_Hub_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
