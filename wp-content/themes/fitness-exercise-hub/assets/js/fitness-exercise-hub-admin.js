        ...
        jQuery.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'fitness_exercise_hub_dismissed_notice_handler',
              type: type,
              wpnonce: fitness_exercise_hub.wpnonce
            }
          } );
      } );
}( jQuery ) )
    ...
    if (closeBtn && detailBox) {
        closeBtn.addEventListener("click", function(e) {
            ...
            fetch(ajaxurl, {
                ...
                body: 'action=fitness_exercise_hub_dismissed_get_started_detail_notice'
            })
...
function fitness_exercise_hub_open_tab(evt, tabName) {
    let i, tabcontent, tablinks;

    // Hide all tabs
    tabcontent = document.getElementsByClassName("fitness-exercise-hub-tabcontent");
    for (i = 0; i < tabcontent.length; i++) {
        tabcontent[i].style.display = "none";
    }

    // Remove active from all buttons
    tablinks = document.getElementsByClassName("fitness-exercise-hub-tablinks");
...
jQuery(document).ready(function ($) {
    // Hide all tabs first
    $(".fitness-exercise-hub-tabcontent").hide();
    
    // **DIRECTLY** activate first tab (no trigger)
    let firstTab = $(".fitness-exercise-hub-tablinks").first();
